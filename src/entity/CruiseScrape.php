...
class CruiseScrape extends Core\EntityHybrid
{
	...
	{
		if (!isset(self::$props[$fieldName])) {
			self::error("cannot retrieve field def for [$fieldName]");
			return null;
		}
		$field = self::$props[$fieldName];
...
//        self::debug("extractng cruiseline logo");
	...
	{
		/*
		 * we now want to make use of the Entity/Model model instead of the database
		 */
		...
		*/
		/*
		 * a simple mapping here might be useful
		 */
		$cruiseline = new Entity\CruiseLine();
		$cruiseline->name = $this->cruiseline;
		$cruiseline->logo=$this->logo;
